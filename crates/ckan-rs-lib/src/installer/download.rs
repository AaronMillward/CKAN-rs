...
pub async fn download_modules_content<'info>(download_directory: &std::path::Path, client: &reqwest::Client, modules: &'info [crate::metadb::ModuleInfo]) -> Vec<(&'info crate::metadb::ModuleInfo, Result<std::path::PathBuf, DownloadError>)> {
